...
static const double w = 0.1;

static const int width = 848;
static const int height = 480;

TEST(TestFov, Basic) {
  Fov::Intrinsicsd intrinsics;
  intrinsics << fx, fy, cx, cy, w;
  Fov cam_model(intrinsics);
...
TEST(TestFov, Jacobian) {
  ...
  using JacobianWrtProjectionParams =
      CamModel::JacobianMatrix<double, 2, CamModel::kNumProjectionParams>;
  using JacobianWrtDistortionParams =
      CamModel::JacobianMatrix<double, 2, CamModel::kNumDistortionParams>;
